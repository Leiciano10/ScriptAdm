...
screen -dmS getpy python ${SCPinst}/PGet.py -b "0.0.0.0:$1" -p "${SCPinst}/pwd.pwd"
 ...
 msg -bar
 echo -ne "$(fun_trans "Su Contraseña Gettunel"):"
 echo -e "\033[1;32m ADMMANAGER"
 } || {
msg -bar
msg -ama "$(fun_trans "Gettunel no se inicio")"
msg -bar
 }
}
pid_kill () {
...
msg -ne " $(fun_trans "Introduzca la opción"): " && read portproxy
tput cuu1 && tput dl1
done
 case $portproxy in
    7)remove_fun && return;;
...
msg -ama "$(fun_trans "Selecciona la puerta en la que los socks van a escuchar")"
msg -bar
porta_socket=
while [[ -z $porta_socket || ! -z $(mportas|grep -w $porta_socket) ]]; do
msg -ne " $(fun_trans "Introduzca el puerto"): " && read porta_socket
tput cuu1 && tput dl1
done
msg -ama " $(fun_trans "Seleccione un texto de conexión")"
msg -bar
msg -ne " $(fun_trans "Introduzca el texto de estado"): " && read texto_soket
    case $portproxy in
