...
msg -ne "$(fun_trans "Nuevas Puertas"): "
read -p "" newports
...
sleep 1s
msg -bar
msg -azu "$(fun_trans "PUERTAS REDEFINIDAS")"
msg -bar
}
edit_apache () {
msg -azu "$(fun_trans "REDEFINIR PUERTAS APACHE")"
msg -bar
local CONF="/etc/apache2/ports.conf"
local NEWCONF="$(cat ${CONF})"
msg -ne "$(fun_trans "Nueva puerta"): "
read -p "" newports
for PTS in `echo ${newports}`; do
...
edit_openvpn () {
msg -azu "$(fun_trans "REDEFINIR PUERTAS OPENVPN")"
msg -bar
...
edit_dropbear () {
msg -azu "$(fun_trans "REDEFINIR PUERTAS DROPBEAR")"
msg -bar
...
edit_openssh () {
msg -azu "$(fun_trans "REDEFINIR PUERTAS OPENSSH")"
msg -bar
...
[[ $apache = $a ]] && echo -ne "\033[1;32m [$apache] > " && msg -azu "$(fun_trans "REDEFINIR PUERTO APACHE")"
[[ $openvpn = $a ]] && echo -ne "\033[1;32m [$openvpn] > " && msg -azu "$(fun_trans "REDEFINIR PUERTO OPENVPN")"
...
msg -bar
while true; do
echo -ne "\033[1;37m$(fun_trans "seleccionar"): " && read selection
tput cuu1 && tput dl1
