...
if [[ ! -z "$pid_fail" ]]; then
 ...
 echo -e "${barra}"
  while [[ -z ${logxyz} || ${logxyz} != @(1|2) ]]; do
   echo -ne "\033[1;37m$(fun_trans "Seleccione la opcion"): " && read logxyz
   tput cuu1 && tput dl1
  done
 case ${logxyz} in
...
echo -e "$barra"
  while [[ -z ${fail2ban} || ${fail2ban} != @(s|S|n|N|y|Y) ]]; do
   echo -ne "\033[1;37m$(fun_trans "Seleccione la opcion") [S/N]: " && read fail2ban
   tput cuu1 && tput dl1
  done
if [[ "$fail2ban" = @(s|S|y|Y) ]]; then
apt-get install fail2ban -y &> /dev/null
wget -O $HOME/fail2ban https://www.dropbox.com/s/qtz4aihjnwpth7y/fail2ban-0.9.4.tar.gz?dl=0 &> /dev/null
...
echo -e "${cor[5]} $(fun_trans "Fail2ban será activo en los siguientes puertos y servicios"):"
echo -e "${barra}"
[ "$s1" != "" ] && echo -ne " $s1"
[ "$s2" != "" ] && echo -ne " $s2"
[ "$s3" != "" ] && echo -ne " $s3"
